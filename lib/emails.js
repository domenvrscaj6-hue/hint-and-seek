...
const SECTION_LABELS = {
  needs: "Things they could really use",
  wants: "Things they'd love",
  likes: "Directions worth exploring"
};
...
function hintList(items, theme) {
  return items.map(h =>
    `<tr><td style="padding:6px 0 6px 4px;color:#a4443a;vertical-align:top;">✦</td>` +
...
function sectionsHtml(hints, theme) {
  return ["needs", "wants", "likes"]
    .filter(k => hints[k] && hints[k].length)
    .map(k => `
      <h3 style="font-family:Georgia,serif;font-size:15px;letter-spacing:.14em;text-transform:uppercase;
                 color:${theme.accent};margin:28px 0 4px;">${SECTION_LABELS[k]}</h3>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">${hintList(hints[k], theme)}</table>
    `).join("");
}
...
export function buildHintEmail({ senderName, occasion, hints, specialNotes, unsubscribeUrl }) {
  ...
  const footer = `
        ...
        this message and don't share it with anyone. The exact wishes behind these hints stay private.
...
export function buildInviteEmail({ requesterName, occasion, siteUrl, unsubscribeUrl }) {
  ...
  const body = `
        ...
        <p style="font-size:17px;line-height:1.65;color:#5c4a33;margin:16px 0 0;">
          Take three minutes and jot down a few things you need, want or simply like.
          Don't worry about spoiling anything: <em>${name} will only see gentle hints, never your exact wishes</em> —
          the surprise stays a surprise for both of you.
        </p>
